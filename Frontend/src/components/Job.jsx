  ...
  return (
    <div className="p-4 md:p-5 rounded-md shadow-xl bg-white border border-gray-100">
      <div className="flex items-center justify-between">
        <p className='text-gray-500 text-xs md:text-sm'>{daysAgoFunction(job?.createdAt)===0?"Today":`${daysAgoFunction(job?.createdAt)}days ago`}</p>
      ...
      <div className="flex items-center gap-2 my-2">
        <Button className="p-4 md:p-6" variant="outline" size="icon">
          <Avatar>
            <AvatarImage 
              src={job?.company?.logo || "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTwB7tzNRxFtDp6rGpzeU_mCel7Dd0DllnlWA&s"} 
      ...
      <div>
        <h1 className='text-base md:text-lg font-bold my-2 line-clamp-1'>{job?.title}</h1>
